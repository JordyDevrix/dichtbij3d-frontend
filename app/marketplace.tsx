...
export default function MarketplaceScreen() {
  ...
  const [filters, setFilters] = useState<Filters>(() => {
    if (searchParams.type && ADVERT_TYPES.includes(searchParams.type as AdvertType)) {
      return { ...EMPTY_FILTERS, types: [searchParams.type as AdvertType] };
    }
  ...
  // Sync if type parameter changes dynamically
  useEffect(() => {
    if (searchParams.type && ADVERT_TYPES.includes(searchParams.type as AdvertType)) {
      setFilters((prev) => {
        const target = searchParams.type as AdvertType;
        if (prev.types.length === 1 && prev.types[0] === target) return prev;
        return { ...prev, types: [target] };
      });
    }
  }, [searchParams.type]);
